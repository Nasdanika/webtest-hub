...
public class UsersControllerGenerator {
  ...
  protected final String TEXT_3 = ";" + NL + "\t\t\t\t$scope.userList = userList;" + NL + "\t\t\t\t" + NL + "\t\t\t\t$scope.updateUser = function(userInfo) {" + NL + "\t\t\t\t\t$scope.userModel.clear();" + NL + "\t\t\t\t\t$scope.userModel.data = userInfo;" + NL + "\t\t\t\t\t$scope.userDialogTitle = \"Update user\";" + NL + "\t\t\t\t\tjQuery(\"#createUpdateUserFormModal\").modal('show');" + NL + "\t\t\t\t}" + NL + "\t\t\t\t" + NL + "\t\t\t\t$scope.createUser = function() {" + NL + "\t\t\t\t\t$scope.userModel.clear();" + NL + "\t\t\t\t\t$scope.userDialogTitle = \"Create user\";" + NL + "\t\t\t\t\tjQuery(\"#createUpdateUserFormModal\").modal('show');" + NL + "\t\t\t\t}" + NL + "\t\t\t\t" + NL + "\t\t\t\t$scope.createOrUpdateUser = function() {" + NL + "\t\t\t\t\tshowFormOverlay();" + NL + "\t\t\t\t\t" + NL + "\t\t\t\t\t$scope.userModel.validateAndApply(hub).then(" + NL + "\t\t\t\t\t\tfunction(userList) {" + NL + "\t\t\t\t\t\t\t$scope.$apply(function($scope) {" + NL + "\t\t\t\t\t\t\t\t$scope.userList = userList;" + NL + "\t\t\t\t\t\t\t\thideFormOverlay();" + NL + "\t\t\t\t\t\t\t\tjQuery(\"#createUpdateUserFormModal\").modal('hide');\t\t\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t\t});\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t}," + NL + "\t\t\t\t\t\tfunction(reason) {" + NL + "\t\t\t\t\t\t\tconsole.dir(reason);" + NL + "\t\t\t\t\t\t\tconsole.dir($scope.userModel);\t\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t\tif (reason.validationFailed) {" + NL + "\t\t\t\t\t\t\t\t$scope.$apply();" + NL + "\t\t\t\t\t\t\t} else {" + NL + "\t\t\t\t\t\t\t\tif (reason.targetInvocationError) {" + NL + "\t\t\t\t\t\t\t\t\tif (typeof reason.targetInvocationError === 'string') {" + NL + "\t\t\t\t\t\t\t\t\t\t$scope.userModel.validationResults['$this'] = reason.targetInvocationError;" + NL + "\t\t\t\t\t\t\t\t\t} else {" + NL + "\t\t\t\t\t\t\t\t\t\t$scope.userModel.validationResult = JSON.stringify(reason.targetInvocationError);\t\t\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t\t\t\t}\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t\t\t} else if (typeof reason === 'string') {" + NL + "\t\t\t\t\t\t\t\t\t$scope.userModel.validationResults['$this'] = reason;" + NL + "\t\t\t\t\t\t\t\t} else {" + NL + "\t\t\t\t\t\t\t\t\t$scope.userModel.validationResults['$this'] = JSON.stringify(reason);" + NL + "\t\t\t\t\t\t\t\t}\t\t\t\t" + NL + "\t\t\t\t\t\t\t\t$scope.$apply();" + NL + "\t\t\t\t\t\t\t}\t\t\t\t\t" + NL + "\t\t\t\t\t\t\thideFormOverlay();" + NL + "\t\t\t\t\t}).done();" + NL + "\t\t\t\t}" + NL + "\t\t\t\t" + NL + "\t\t\t\t$scope.deleteUser = function(userInfo) {" + NL + "\t\t\t\t\tif (confirm(\"Do you really want to delete user \"+userInfo.login+\"?\")) {" + NL + "\t\t\t\t\t\tshowOverlay();" + NL + "\t\t\t\t\t\thub.deleteUser(userInfo.login).then(updateUserList, onError).done();" + NL + "\t\t\t\t\t}\t\t\t\t" + NL + "\t\t\t\t}" + NL + "\t\t\t\t \t" + NL + "\t\t\t\thideOverlay();" + NL + "\t\t\t});\t\t\t\t\t\t\t" + NL + "\t\t});\t\t\t   \t\t  \t\t" + NL + "      " + NL + "\t}]);" + NL + "    " + NL + "\tangular.bootstrap(jQuery(\"#usersApp\"), ['hubUsersApp']);" + NL + "});";
